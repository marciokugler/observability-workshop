...
async function runUser(userNumber, roundNumber) {
  ...
  const context = await browser.newContext({
    ...
    userAgent: `support-portal-rum-simulator/${browserName}/${roundNumber}-${userNumber}`,
  ...
  await page.evaluate(
    ([user, round, activeBrowser]) => {
      window.localStorage.setItem("support-portal-sim-user", `user-${round}-${user}`);
      window.sessionStorage.setItem("support-portal-sim-session", crypto.randomUUID());
      window.localStorage.setItem("support-portal-sim-browser", activeBrowser);
    },
