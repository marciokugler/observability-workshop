import { ACTION_TYPES, BUSINESS_TRANSACTIONS, type AssistantEvidenceInput } from "@support-portal/shared-types";
...
function normalizeTransaction(text: string) {
  ...
  if (
    ...
    value.includes("ai support response")
  ) {
...
function inferTransaction(text: string) {
  ...
  return normalized.includes("account") || normalized.includes("case")
    ? BUSINESS_TRANSACTIONS.caseStatusLookup
    : normalized.includes("help") || normalized.includes("article") || normalized.includes("search")
...
function inferCandidateActions(text: string) {
  ...
  if (
    normalized.includes("clean_support_knowledge_cache") ||
    normalized.includes("clean_service_cache") ||
    normalized.includes("clean support knowledge cache") ||
...
function inferConfidence(text: string) {
  ...
  if (
    ...
    ((normalized.includes("support-knowledge") || normalized.includes("support-knowledge")) &&
      (normalized.includes("clean_support_knowledge_cache") || normalized.includes("clean_service_cache")))
  ) {
    return "high";
  }
