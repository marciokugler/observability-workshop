...
import { parseAssistantEvidence } from "@support-portal/evidence-parser";
import { evaluatePolicy } from "@support-portal/policy-engine";
import {
  ACTION_TYPES,
...
export function buildServer() {
  ...
  void app.register(cors, {
    origin: true,
    allowedHeaders: ["content-type", "traceparent", "tracestate", "baggage", "x-support-portal-webhook-secret"],
  ...
  app.post("/webhooks/splunk/detector", async (request, reply) => {
    if (webhookSharedSecret) {
      const providedSecret = request.headers["x-support-portal-webhook-secret"];
...
const demoInput: AssistantEvidenceInput = {
  ...
  rawText:
    "High confidence that support-knowledge cache filesystem pressure degraded the AI Support Response transaction. Disk utilization for the cache mount is above threshold and APM shows support-knowledge latency. Recommended action: clean_support_knowledge_cache.",
  pastedBy: "operator",
  pastedAt: new Date().toISOString()
};
