...
export class SplunkMcpClient {
  ...
  private buildScope(payload: DetectorWebhookPayload): McpScope {
    ...
    return {
      ...
      service: payload.dimensions?.service ?? "support-knowledge",
      environment,
      instance:
  ...
  private buildEvidenceResult(scope: McpScope, records: ToolRecord[], warnings: string[]): SplunkMcpEvidenceResult {
    ...
    const likelyCause =
      confidenceBand === "high"
        ? `${scope.service} cache filesystem pressure is correlated with elevated APM latency for AI Support Response.`
    ...
    return {
      ...
      p95LatencyMs,
      errorRate,
      affectedTransactions: ["support_response"],
...
function extractServiceNames(text: string) {
  return Array.from(text.matchAll(/\bsupport-[a-z0-9-]+\b/gi)).map((match) => match[0]);
}
...
function buildApprovalEvidence(input: {
  ...
  const topologyConfirmed =
    ...
    input.affectedServices.some((service) => service === "support-portal-api" || service === "support-assistant");
    ...
    customerImpact: [
      {
        ...
        detail: input.latencyElevated
          ? `Digital Experience network activity for AI Support Response is slow; correlated backend p95 is ${roundMetric(input.p95LatencyMs) ?? "above threshold"} ms for ${BUSINESS_TRANSACTIONS.customerSupportResponse}.`
    ...
    backendImpact: [
      {
        ...
        status: endpointStatus,
        detail: input.latencyElevated
          ? `APM latency for ${input.scope.service} is above the ${input.thresholds.apmLatencyMs} ms threshold on the support-response path.`
    ...
    impactChain: [
      {
        ...
        detail: "RUM/network evidence identifies POST /api/support/respond as the customer-visible AI Support Response request.",
        status: endpointStatus
      },
      {
        stage: "gateway",
        ...
        detail: topologyConfirmed
          ...
          : "The known route is support-portal-api to support-assistant to support-knowledge; dependency evidence was not returned.",
      ...
      },
      {
        ...
        detail: remediationSupported
          ? "The evidence supports clean_support_knowledge_cache before operator approval."
...
function buildAssistantSummary(input: {
  ...
  const lines = [
    `${input.confidenceBand[0].toUpperCase()}${input.confidenceBand.slice(1)} confidence Splunk MCP evidence for AI Support Response.`,
  ...
  lines.push("Recommended action: clean_support_knowledge_cache.");
  return lines.join("\n");
}
