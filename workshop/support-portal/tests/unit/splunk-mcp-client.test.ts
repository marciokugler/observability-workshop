...
test("SplunkMcpClient gathers service, latency, metric, and trace evidence", async () => {
  ...
  globalThis.fetch = (async (_url, init) => {
    ...
    const textByTool: Record<string, string> = {
      get_apm_services: "support-knowledge support-portal-api",
  ...
  try {
    ...
    assert.equal(evidence.suspectService, "support-knowledge");
      ...
      body.params.name === "execute_signalflow_program"
        ...
        : {
            content: [
              {
                type: "text",
                text: '{"service":"support-knowledge","p95LatencyMs":0.3666666666666667}'
              }
            ]
          };
