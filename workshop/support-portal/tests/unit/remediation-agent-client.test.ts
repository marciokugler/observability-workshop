...
function buildEvidence(): EvidenceBundle {
  return {
    ...
    investigation: {
      likelyCause: "support-knowledge cache filesystem pressure",
      confidenceBand: "high"
    },
...
test("RemediationAgentClient maps agent evaluation into a bounded proposed action", async () => {
  ...
  globalThis.fetch = (async (url, init) => {
    ...
    return new Response(
      JSON.stringify({
        incidentId: "incident-agent-client",
        recommendedAction: "clean_support_knowledge_cache",
        model: "gpt-4.1-mini",
        confidenceBand: "high",
        reasoningSummary: "Clean the bounded support knowledge cache.",
        needsApproval: true
      })
    );
  }) as typeof fetch;
