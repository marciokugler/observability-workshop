# Support Portal Workshop Runbook

This site is the operator guide for running the `Support Portal` workshop from this repository.
...
A company launches an AI-powered support portal. During peak usage, the support knowledge cache volume fills up and degrades the `AI Support Response` workflow. Splunk observability surfaces customer impact, service latency, and filesystem pressure. The remediation orchestrator collects evidence, applies policy, asks for approval, executes `clean_support_knowledge_cache`, and verifies recovery.
